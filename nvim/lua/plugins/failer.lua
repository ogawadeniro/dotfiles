...
local function setup_nvimtree()
    ...
    local MIN_WIDTH = 40
    local MIN_HEIGHT = 15
    local width = math.max(math.floor(gwidth / 2), MIN_WIDTH)
    local height = math.max(math.floor(gheight * 3 / 4), MIN_HEIGHT)
    local row = gheight - height
    local col = gwidth - width
