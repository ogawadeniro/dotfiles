local BufCtl = function()
    ...
    self.delete_buf = function()
        ...
        if bufinfo.is_current == true then
            vim.notify("Current buffer can't delete", vim.log.levels.ERROR, { title = "BufCtl" })
            return
        end
        if bufinfo.is_term == true then
            vim.notify("Terminal buffer can't be delete", vim.log.levels.ERROR, { title = "BufCtl" })
            return
        end
        if bufinfo.is_change == true then
            if self.is_confirmed("Buffer is modified.") == false then
                vim.notify("Canceled buffer deletion", vim.log.levels.INFO, { title = "BufCtl" })
                return
            end
        end
    ...
    self.is_confirmed = function(msg)
        -- local inputstr = vim.notify(msg .. " type y to continue > ", vim.log.levels.WARN)
